...
TEST_F(UrlLoaderTest, DidReceiveDataCrossUpperThreshold) {
  ...
  char read_buffer[1];
  loader_->ReadResponseBody(base::as_writable_byte_span(read_buffer),
                            mock_read_callback_.Get());
  {
...
TEST_F(UrlLoaderTest, ReadResponseBodyWithEmptyBuffer) {
  ...
  loader_->ReadResponseBody(base::span<uint8_t>(), mock_read_callback_.Get());
}
...
TEST_F(UrlLoaderTest, ReadResponseBodyWithSmallerBuffer) {
  ...
  loader_->ReadResponseBody(base::as_writable_byte_span(tail_buffer),
                            mock_read_callback_.Get());
...
TEST_F(UrlLoaderTest, EnablePushMode) {
  ...
  loader_->EnablePushMode(
      ...
      base::BindOnce([](bool* called, int* result, int r) {
        *called = true;
        *result = r;
      }, &complete_called, &complete_result));
